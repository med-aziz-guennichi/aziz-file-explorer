...
fn check_file(matcher: &SkimMatcherV2, accept_files: bool, filename: &String, file_path: &String, extension: &String, query: String, results: &mut Vec<DirectoryChild>, fuzzy_scores: &mut Vec<i16>) {
    if !accept_files {
        return;
    }
    ...
    let cleaned_filename = filename_path.file_stem().and_then(|stem| stem.to_str()).unwrap_or("");
...
#[tauri::command]
pub async fn search_directory(state_mux: State<'_, StateSafe>, query: String, search_directory: String, mount_pnt: String, extension: String, accept_files: bool, accept_directories: bool) -> Result<Vec<DirectoryChild>, ()> {
    ...
    for (filename, paths) in system_cache {
        for path in paths {
            ...
            if file_type == "file" {
                check_file(&matcher, accept_files, filename, file_path, &extension, query.clone(), &mut results, &mut fuzzy_scores);
                continue;
            }
    ...
    Ok(tuples.into_iter().map(|(index, _)| results[index].clone()).collect())
